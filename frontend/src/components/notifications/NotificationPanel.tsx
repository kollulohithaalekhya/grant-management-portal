    ...
    <div className="absolute right-0 top-12 w-96 bg-white rounded-xl shadow-lg border border-gray-200 z-50 overflow-hidden">
      ...
      <div className="max-h-96 overflow-y-auto divide-y">
        {loading ? (
          ...
          notifs.map((n) => (
            <div key={n._id} className={`flex gap-3 p-3 hover:bg-gray-50 ${!n.isRead ? 'bg-blue-50/40' : ''}`}>
              ...
              <div className="flex flex-col gap-1 flex-shrink-0">
                {!n.isRead && (
                  <button onClick={() => markRead(n._id)} className="text-primary-600 hover:text-primary-700">
                    <Check className="w-4 h-4" />
                  </button>
                )}
                <button onClick={() => deleteNotif(n._id)} className="text-gray-400 hover:text-red-500">
